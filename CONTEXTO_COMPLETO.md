...
- Em 26/05/2026 foram incorporados aprendizados das novas correcoes manuais feitas pelo usuario na Nexus e na Mex. A prioridade maxima continua sendo das marcacoes manuais salvas; as regras abaixo so ampliam matching automatico em casos seguros. Arquivo Nexus alterado: `netlify/functions/refresh-prices.js`.
- Regras novas/fortalecidas: typos `digitall`, `digita`, `komba`, `mwii`, `mwiii`, `marvels`; slugs colados como `ps4midia`; limpeza de carimbos de data/hora em slugs; equivalencias conservadoras para The Witcher 3/Wild Hunt, COD Modern Warfare 2019/II/III, PES/eFootball por ano, Little Nightmares 1+2, Rise of the Ronin/A Ascensao do Ronin, Resident Evil 7 Biohazard, Resident Evil Revelations 1+2 e Marvel Spider-Man base/GOTY; trava para impedir Spider-Man base/GOTY de aceitar Miles Morales ou Remastered.
- Validacao dessa rodada: `npm.cmd run check` passou; simulacao aceitou os positivos e rejeitou RE4 2005 vs Remake, Modern Warfare 2 vs III, Spider-Man vs Miles Morales e Elden Ring vs Nightreign. A mesma familia de regras foi aplicada separadamente na Mex, sem misturar dados, ids, Blob Stores, deploys ou Cron Jobs.
- Publicacao/estado de 26/05/2026: Nexus Netlify `6a1552a51be7dd70a9ecb36c` ready; GitHub `f9ec59d`/`d5affd9`; Render Nexus live no commit `d5affd9`; coleta Nexus reiniciada no job `crn-d87t66n7f7vs73dqjnpg-1779782516`, observada `running` em `offset 0/1039`. Mex tambem recebeu a mesma familia de regras separadamente: Netlify `6a1552995acd3084d700c81c`, GitHub `e13f30f`/`8723db0`, Render live, coleta Mex `crn-d8a5v3n7f7vs73cqt210-1779782607`.
- Ainda em 26/05/2026, ao retomar o projeto, a coleta Nexus parcial mostrou falsos positivos automaticos de edicao/pacote: `Modern Warfare` base contra `Modern Warfare Remastered`, `Resident Evil 8: Village` base contra `Village Gold`, e pacote `Resident Evil 7 Gold Edition & Village Gold Edition` contra `Village` simples. Foram corrigidos `app.js` e `netlify/functions/refresh-prices.js`: o coletor agora exige presenca compativel de `remaster/remastered` em Call of Duty Modern Warfare, exige compatibilidade de `gold` e `7` nos equivalentes de Resident Evil Village, e o painel ignora pares automaticos nao confirmados quando ha divergencia de edicao/pacote. Confirmacoes manuais continuam com prioridade. Validacoes: `npm.cmd run check` passou; simulacao local bloqueou os falsos positivos e manteve os positivos; Netlify `6a155b7c5acd309e7600c877` ready; GitHub `aa3d906`; Render `dep-d8alk43eo5us73bolr8g` live no commit `aa3d906`; coleta antiga `crn-d87t66n7f7vs73dqjnpg-1779782516` pausada em `234/1039`; nova coleta limpa `crn-d87t66n7f7vs73dqjnpg-1779784851` iniciada e observada `running`, `offset 0/1039`, lendo catalogos dos concorrentes.
