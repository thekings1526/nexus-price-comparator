...
- Ainda em 25/05/2026 os ajustes de comparacao cruzada foram publicados. GitHub: `7af62fc` para `netlify/functions/refresh-prices.js`, `162622f` para `RETOMADA.md` e `ddcd2df` para `CONTEXTO_COMPLETO.md`. Netlify: deploy `6a1452a31fc19b3184e2d70e`, publicado com estaticos + funcoes usando SHA1 para arquivos e SHA256 + `runtime=js` para funcoes. Render: Cron Job `nexus-price-worker` com auto deploy `yes`, ultimo deploy `dep-d8a50vcubeas73b71g4g` live no commit `ddcd2df`. Validacao publicada: painel `200`, `/app.js` `200`, `/api/report` `200` com `1039/1039` e status `ready`, `/api/review-decision` `200` com `126` exemplos de aprendizado, `/api/review-candidates` respondeu `200`.
