...
- Ainda em 25/05/2026 foi feita uma auditoria geral das confirmacoes/aprendizado e dos produtos comparados. O Blob `match-overrides.learning` tinha 118 exemplos, sendo 114 positivos e 4 negativos. A auditoria separou falsos alertas legitimos de numerais/anos (`VII/7`, `II/2`, `FIFA 23/2023`) e encontrou 6 linhas com erro forte no relatorio salvo: `Horizon Forbidden West PS4` na NGCP apontando para URL/titulo PS5, e jogos `F1 22/F1 23` normais apontando para `F1 Manager` na Coelho/NGCP. Foi adicionada regra para o coletor bloquear `F1` normal contra `F1 Manager` e regra no painel para ignorar imediatamente, no calculo, pares automaticos com conflito claro de plataforma ou `F1 Manager` sem mexer em confirmacoes manuais. Arquivos alterados: `app.js` e `netlify/functions/refresh-prices.js`. Simulacao no relatorio publicado indicou exatamente 6 linhas novas ignoradas e nenhum falso positivo em produtos `PS4/PS5`.
